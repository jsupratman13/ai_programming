...
class DPMatching(object):
	def __init__(self, template, unknown):
		...
		self.d = np.empty([self.i, self.j])*0
		self.g = np.empty([self.i, self.j])*0
	...
	def local_dist(self):
		...
		for i in range(self.i):
			for j in range(self.j):
				for k in range(15):
					self.d[i,j] += math.pow(u[i,k]-t[j,k],2)
				self.d[i,j] = math.sqrt(self.d[i,j])
...
def dp_match(template, unknown):
	...
	d = np.empty([140,140])*0
	g = np.empty([140,140])*0
	for i in range(I):
		for j in range(J):
			for k in range(15):
