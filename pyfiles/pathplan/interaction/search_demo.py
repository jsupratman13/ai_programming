...
class Grid(object):
	...
	def use_grid2(self):
		W = self.WHITE
		grid_map = [[W for col in range(35)] for row in range(25)]
		self.grid_map = grid_map
		return grid_map
...
if __name__ == '__main__':
	...
	rect = im.get_rect()
	while rect.width > WIDTH-7 and rect.height > HEIGHT-7:
		w = rect.width * .9
		h = rect.height * .9
		im = pygame.transform.scale(im, (int(w), int(h)))
		rect = im.get_rect()
	init_x = 0
	init_y = 0
	...
	rect2 = im2.get_rect()
	while rect2.width > WIDTH and rect2.height > HEIGHT:
		w = rect2.width*.9
		h = rect2.height*.9
		im2 = pygame.transform.scale(im2, (int(w),int(h)))
		rect2 = im2.get_rect()
